    ...
    while (left_i < left_len) & (right_i < right_len):
        if left[left_i][0] != right[right_i][0]:
            ...
            if how == 'left' or how == 'outer':
                new_values.append((left[left_i][0], *left[left_i][1:], *(None,) * right_column_count))
                left_i += 1
            elif how == 'right' or how == 'outer':
                new_values.append((right[right_i][0], *(None,) * left_column_count, *right[right_i][1:]))
                right_i += 1


        elif left[left_i][0] == right[right_i][0]:
            # append the line whatever the join method
            new_values.append((left[left_i][0], *left[left_i][1:], *right[right_i][1:]))
            right_i += 1
